/**
 * 为所有年级（1-12）每个补充100道题（直接 MySQL 版）
 * 用法: npx tsx scripts/seed-all-questions-v2.ts
 */
...
function randChoice(a: number, range: number, count: number): string[] {
  ...
  let maxAttempts = 100
  while (answers.size < count && maxAttempts-- > 0) {
    const offset = Math.floor(Math.random() * range * 2) - range
    const val = a + offset
    if (val !== a) answers.add(String(val))
  }
...
function makeG12Questions(): QuestionInput[] {
  ...
  for (let i = 0; i < 16; i++) {
    const isChoice = i % 2 === 0
    const a = Math.floor(Math.random() * 4) + 3
    const b = Math.floor(Math.random() * (a - 1)) + 2  // 确保 a > b，椭圆中 a 为长半轴
...
async function main() {
  ...
  try {
    ...
    console.error('[FAIL] 数据库连接失败:', e.message)
    process.exit(1)
  }

  const generators: Record<number, () => QuestionInput[]> = {
    1: makeG1Questions, 2: makeG2Questions, 3: makeG3Questions,
    4: makeG4Questions, 5: makeG5Questions, 6: makeG6Questions,
    7: makeG7Questions, 8: makeG8Questions, 9: makeG9Questions,
    10: makeG10Questions, 11: makeG11Questions, 12: makeG12Questions,
  }
  ...
  for (let grade = 1; grade <= 12; grade++) {
    ...
    console.log(`[INFO] 年级 ${grade}：准备导入 ${unique.length} 道题目...`)
    ...
    for (let i = 0; i < unique.length; i++) {
      ...
      const difficultyScore = { 1: 2, 2: 5, 3: 8 }[q.difficulty] ?? 5
      ...
      try {
        await pool.query(
          `INSERT INTO t_question (id, level_id, type, knowledge_point, difficulty, prompt, \`options\`, answer, explanation, xp, illustration, difficulty_score)
           ...
           ON DUPLICATE KEY UPDATE
             level_id = VALUES(level_id), type = VALUES(type),
             knowledge_point = VALUES(knowledge_point), difficulty = VALUES(difficulty),
             prompt = VALUES(prompt), \`options\` = VALUES(\`options\`),
             answer = VALUES(answer), explanation = VALUES(explanation),
             xp = VALUES(xp), illustration = VALUES(illustration),
          ...
          [id, q.levelId, q.type, q.knowledgePoint, q.difficulty,
           ...
           Number(q.xp) || 10, null, difficultyScore]
        )
        success++
        if ((i + 1) % 20 === 0) {
        ...
        fail++
        console.error(`  [FAIL] 失败: ${q.prompt}`, err.message)
      }
    }

    console.log(`  [OK] 年级 ${grade}：成功 ${success} 道，失败 ${fail} 道`)
