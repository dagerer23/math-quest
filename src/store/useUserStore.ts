...
function applyStreak(state: UserState): UserState {
  ...
  return { ...state, streak: newStreak, lastActiveDate: today, dailyGoals: [], dailyGoalDate: today, dailyXp: 0, dailyQuestions: 0 }
}
  ...
  persist(
    (set, get) => ({
      ...
      registerSession: (record) => {
        ...
        const newMistakes = record.answers.filter((a) => !a.isCorrect).map((a) => a.questionId).filter((id) => !state.mistakeIds.includes(id))
        ...
        if (!hasAchievement('first_blood') && record.stars >= 1) {
        ...
        if (record.stars >= 1) {
          ...
          if (!state.unlockedLevels.includes(gradeKey)) {
            set({ unlockedLevels: [...state.unlockedLevels, gradeKey] })
          }
        }
        ...
        const next: UserState = applyStreak({
          ...
          comboMax: newCombo,
          completedLevels: { ...get().completedLevels, [record.levelId]: { stars: record.stars, bestScore: record.score } },
          mistakeIds: [...get().mistakeIds, ...newMistakes],
          achievements: [...get().achievements, ...achievementUnlocks.map((a) => ({ id: a.id, unlockedAt: Date.now() }))],
          learningStats: { ...get().learningStats, totalQuestions: get().learningStats.totalQuestions + record.totalCount, correctQuestions: get().learningStats.correctQuestions + record.correctCount },
          dailyQuestions: get().dailyQuestions + record.totalCount,
        })
        set(next)
        return { newUnlocks: achievementUnlocks, leveledUp: state.rank !== newRank }
      },
      addMistake: (id) => { if (!get().mistakeIds.includes(id)) set({ mistakeIds: [...get().mistakeIds, id] }) },
      ...
      incrementMistakeMastery: (id) => {
        ...
        if (next >= 3) {
          set({ mistakeMastery: { ...get().mistakeMastery, [id]: next }, mistakeIds: get().mistakeIds.filter((m) => m !== id) })
        } else {
      ...
      completeOnboarding: () => {
        ...
        if (!Number.isFinite(g) || g < 1) { set({ hasCompletedOnboarding: true }); return }
        const gradeKey = `g${g}-L1` as const
        const currentUnlocked = get().unlockedLevels
        const nextUnlocked = currentUnlocked.includes(gradeKey) ? currentUnlocked : [...currentUnlocked, gradeKey]
        set({ hasCompletedOnboarding: true, grade: g, profile: { ...get().profile, targetGrade: g }, unlockedLevels: nextUnlocked })
      },
      ...
      completeDailyGoal: (goalId) => {
        ...
        if (goal && !goal.completed && goal.reward) {
          set((state) => ({ dailyGoals: state.dailyGoals.map((g) => g.id === goalId ? { ...g, completed: true, completedAt: Date.now() } : g), xp: state.xp + goal.reward!.xp, coins: state.coins + goal.reward!.coins }))
        }
      },
      addInventoryItem: (item) => {
        set((state) => {
          const existing = state.inventory.find((i) => i.id === item.id)
          if (existing) return { inventory: state.inventory.map((i) => i.id === item.id ? { ...i, count: i.count + item.count } : i) }
      ...
      useInventoryItem: (itemId) => {
        ...
        set((s) => ({ inventory: s.inventory.map((i) => i.id === itemId ? { ...i, count: i.count - 1 } : i).filter((i) => i.count > 0) }))
        return true
      },
      addTreasureBox: (box) => set({ treasureBoxes: [...get().treasureBoxes, box] }),
      openTreasureBox: (boxId) => {
        const box = get().treasureBoxes.find((b) => b.id === boxId)
        if (!box || box.opened) return null
        set((s) => ({ treasureBoxes: s.treasureBoxes.map((b) => b.id === boxId ? { ...b, opened: true, openedAt: Date.now() } : b), xp: s.xp + box.rewards.xp, coins: s.coins + box.rewards.coins }))
        return box.rewards
      },
      updateLearningStats: (stats) => set((state) => ({ learningStats: { ...state.learningStats, ...stats } })),
      reset: () => set({ ...initial, lastActiveDate: todayKey() }),
      checkIn: () => {
        const state = get()
        const today = todayKey()
        if (state.lastCheckInDate === today) return { success: false, message: '今日已签到，请明天再来！' }
        let newStreak = state.streak
        if (isYesterday(state.lastCheckInDate || '')) newStreak = state.streak + 1
        else if (state.lastCheckInDate !== today) newStreak = 1
        ...
        const rewards = { xp: baseXp + newStreak * xpPerStreak, coins: baseCoins + newStreak * coinsPerStreak }
        set({ lastCheckInDate: today, streak: newStreak, xp: state.xp + rewards.xp, coins: state.coins + rewards.coins })
        return { success: true, message: `签到成功！获得 ${rewards.xp} XP 和 ${rewards.coins} 金币，连续签到 ${newStreak} 天！` }
      },
      setUserId: (userId: string) => { set({ userId }) },
      loginWithPhone: (phone: string, userId: string) => { set({ isLoggedIn: true, userId, lastLoginAt: Date.now(), profile: { ...get().profile, phone } }) },
      resetUserIdentity: () => { set({ hasCompletedOnboarding: false, assessment: undefined, profile: { avatar: '', nickname: '', learningStage: 'primary', learningGoal: 'consolidation', targetGrade: undefined as unknown as number, phone: get().profile.phone }, grade: undefined as unknown as number, unlockedLevels: [], completedLevels: {} }) },
      logout: () => { const currentProfile = get().profile; set({ isLoggedIn: false, userId: undefined, lastLoginAt: undefined, hasCompletedOnboarding: false, assessment: undefined, profile: { ...currentProfile } }) },
      setPhone: (phone: string) => { set({ profile: { ...get().profile, phone } }) },
      ...
      setSystemConfigs: (configs) => {
        ...
        set({ systemConfigs: configs, ...(isFirstLoad ? { coins: initCoins, diamonds: initDiamonds, hearts: initHearts, maxHearts } : { maxHearts }) })
      },
    }),
