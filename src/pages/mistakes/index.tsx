...
export default function MistakesPage() {
  ...
  const mistakes = mistakeIds.map((id) => findQuestion(id)).filter(Boolean)
        ...
        mistakes.map((q: any, idx: number) => (
          <Card key={q.id} padding={16} style={{ margin: 8, marginTop: 8 }}>
            <Row justify="space-between">
              <Col gap={6} flex={1}>
                <Text style={{ fontSize: 14, color: '#6b7280' }}>第 {idx + 1} 题 · {q.knowledgePoint || '综合'}</Text>
                <Text style={{ fontSize: 16, fontWeight: 600, color: '#1a1a1a', lineHeight: 1.5 }}>{q.prompt}</Text>
                <Text style={{ fontSize: 13, color: '#58CC02', marginTop: 4 }}>✅ 正确答案：{q.answer}</Text>
                {q.explanation && <Text style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>💡 {q.explanation}</Text>}
              </Col>
