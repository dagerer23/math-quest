...
export function Card({ children, padding = 24, style, onClick }: CardProps) {
  return (
    <View onClick={onClick} style={{ background: '#FFFFFF', borderRadius: 16, padding: padding as any, boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 2px 8px rgba(0,0,0,0.04)', border: '1px solid #f0f0f0', ...style }}>
      {children}
    </View>
  )
}
...
export function Title({ children, size = 20, weight = 700, color = '#1a1a1a', style }: TitleProps) {
  return <Text style={{ fontSize: size, fontWeight: weight as any, color, ...style }}>{children}</Text>
}
