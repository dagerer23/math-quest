        ...
        <Row justify="space-between">
          <Col gap={4}>
            <Text style={{ fontSize: 22, fontWeight: 800, color: '#FFF' }}>
              你好，{user.profile.nickname || '小同学'}！
            </Text>
            <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.9)' }}>
              {getGradeName(grade)} · 今日已练习 {user.dailyQuestions} 题
            </Text>
          </Col>
          <Col gap={4} style={{ alignItems: 'flex-end' }}>
            <Row gap={12}>
              <Col style={{ alignItems: 'center' }} gap={0}>
                <Text style={{ fontSize: 20, fontWeight: 700, color: '#FFF' }}>{user.xp}</Text>
                <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.8)' }}>XP</Text>
              </Col>
              <Col style={{ alignItems: 'center' }} gap={0}>
                <Text style={{ fontSize: 20, fontWeight: 700, color: '#FFF' }}>{user.coins}</Text>
                <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.8)' }}>金币</Text>
              </Col>
              <Col style={{ alignItems: 'center' }} gap={0}>
                <Text style={{ fontSize: 20, fontWeight: 700, color: '#FFF' }}>❤️{user.hearts}</Text>
                <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.8)' }}>体力</Text>
              </Col>
            </Row>
          </Col>
        </Row>
            ...
            <View key={level.id}>
              <Card
                ...
                style={{
                  ...
                  background: level.isBoss ? '#FFF7ED' : '#FFFFFF',
                  border: level.isBoss ? '2px solid #F59E0B' : undefined,
                }}
              >
                <Row justify="space-between">
                  <Col gap={8} flex={1}>
                    <Row gap={8}>
                      <View
                        style={{
                          ...
                          height: 44,
                          borderRadius: 22,
                          background: unlocked ? (level.isBoss ? '#F59E0B' : '#58CC02') : '#D1D5DB',
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      <Col gap={2} flex={1}>
                        ...
                        <Text style={{ fontSize: 12, color: '#6b7280' }}>
                          {level.description || `${level.knowledgePoints?.join(' · ') || '综合练习'}`}
                        </Text>
                        <Row gap={6} style={{ marginTop: 4 }}>
                          {[1, 2, 3].map((s) => (
                            <Text key={s} style={{ fontSize: 14 }}>
                              {s <= stars ? '⭐' : '☆'}
                            </Text>
                          ))}
                        </Row>
